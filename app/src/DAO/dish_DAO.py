...
class DishDAO:
    ...
    def create_dish(api_test_menu_id, api_test_submenu_id, data, db: Session):
        dish = Dish(menu_id=api_test_menu_id, submenu_id=api_test_submenu_id,  title=getattr(data, 'title'), description=getattr(data, 'description'), price=getattr(data, 'price'))
        db.add(dish)
        db.commit()
        db.refresh(dish)
