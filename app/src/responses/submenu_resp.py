...
class Submenu_resp:
    ...
    def get_submenus(api_test_menu_id, db: Session):
        ...
        if submenus is not None:
            ...
            r.set(json.dumps([api_test_menu_id, "submenus"]), json.dumps(submenus))
            return submenus
    ...
    def get_submenu(api_test_menu_id, api_test_submenu_id, db: Session):
        ...
        if submenu is not None:
            ...
            submenu = SubmenuDAO.get_submenu(api_test_menu_id, api_test_submenu_id, db)
            r.set(json.dumps([api_test_menu_id, api_test_submenu_id]), json.dumps(submenu))
            if submenu == None:
    ...
    def create_submenu(api_test_menu_id, data, db: Session):
        r.delete(json.dumps([api_test_menu_id, "submenus"]), api_test_menu_id, "menus")
    ...
    def edit_submenu(api_test_menu_id, api_test_submenu_id, data, db: Session):
        r.delete(json.dumps([api_test_menu_id, "submenus"]), json.dumps([api_test_menu_id, api_test_submenu_id]), api_test_menu_id, "menus")
        submenu = SubmenuDAO.edit_submenu(api_test_menu_id, api_test_submenu_id, data, db)
        if submenu == None:
            return JSONResponse(status_code=404, content={"detail": "submenu not found"})
        else:
            return submenu
